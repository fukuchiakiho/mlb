...
class RNN:
    def __init__(self, Wx, Wh, b):
        ...
        #self.grads = [np.zeros_like(Wx).tolist(), np.zeros_like(Wh).tolist(), np.zeros_like(b).tolist()]
        self.cache = None
    ...
    def forward(self, x, h_prev):
        ...
        #t = sum(mul, h_prev, Wh) + sum(mul, h_prev, Wx) + b
        h_next = np.tanh(t)
        #h_next = math.tanh(t)
...
class TimeAffine:
    def __init__(self, W, b):
        ...
        #self.grads = [np.zeros_like(W).tolist(), np.zeros_like(b).tolist()]
        self.x = None
...
class TimeSoftmaxWithLoss:
    ...
    def forward(self, xs, ts):
        ...
        if len(ts) == 3:  # 教師ラベルがone-hotベクトルの場合
            ts = ts.argmax(axis=2)
